  ...
  return (
    <section className="border-y border-border bg-white py-16 md:py-20">
      <div className="container">
          ...
          {headline.map((stat, i) => (
            <Reveal key={stat.id} delay={i * 0.08}>
              <div className="text-center sm:text-left">
                ...
                <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{stat.caption}</p>
        ...
        <Reveal delay={0.3}>
          <div className="mt-14 grid gap-6 rounded-2xl bg-background p-6 sm:grid-cols-2 lg:grid-cols-4 lg:p-8">
            <RegionStat
...
function BenchmarkBar({
  ...
  return (
    <div className="mt-4 space-y-2">
      <Bar width={oursPct} label="Us" tone={better ? 'gold' : 'muted'} />
      <Bar width={theirsPct} label="KW avg" tone="muted" />
    </div>
  );
}
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-sand-dark">
        <motion.div
          ...
          className={tone === 'gold' ? 'h-full bg-gradient-to-r from-gold to-gold-bright' : 'h-full bg-slate-ink/25'}
        />
      </div>
    </div>
  );
